...
const UserProfile = () => {
  ...
  const localStorageUser = JSON.parse(localStorage.getItem('user'));  // Get user from localStorage

  const getUser = async (req, res) => {
    try {
      const response = await axios.get('http://localhost:5001/api/user',localStorageUser);

      return user = JSON.stringify(response.data.user);
    } catch (error) {
      alert(err.response?.data?.message || "user data not found.");
    };
  };
